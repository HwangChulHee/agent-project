...
## 2026-06-08 — "Act" 입도: 원시어 필터 넣었다 되돌림 (하드코딩 규율)

**실측:** "Act"는 방법섹션 9회 0, 전 11섹션 1회 — 드물고 간헐적, 체계적 입도붕괴 아님. 메서드 분해 원시어가 가끔 단독 누출.
**시도→철회:** `_GENERIC_PRIMITIVES` 단어 리스트 필터를 넣었다가(단위 20/20) **하드코딩 누적이 냄새**라 revert. **규율 정립:** 하드코딩 필터는 *유한·알려짐·안정적* 집합에만(모델 인스턴스 = OK, 프롬프트가 이미 열거). 원시어/개념성처럼 *열린* 판단은 단어 리스트 금지.
**원칙적 해법(열린 설계):** **개념성(concept-hood)도 "코퍼스 재등장"으로** — 진짜 개념은 여러 논문에 재등장·정의됨, 노이즈(스친 'Act', 실험용 'GPT-4')는 한 번 나오고 정의 안 됨. 정책의 "맵 위상에서 떠오른다"와 같은 정신. **논문 더 쌓이면 "정의됨 vs 언급만 / 재등장" 신호로 구현.** 그때까지 'Act'류 드문 노이즈 감수. (모델 필터는 유지)
**다음 행동:** 논문 추가해 다중 코퍼스 실측 — concept-hood 재등장, merge ①, 오염 enrichment, builds_on 교차검증을 한꺼번에 시험. 후보는 ToT/ReAct 인용 lineage(CoT 2201.11903 / Self-Consistency 2203.11171 / STaR / Least-to-Most / Reflexion 등).

## 2026-06-08 — 04 모델 인스턴스 필터 (추출 노이즈)

**문제:** 04 프롬프트가 `GPT-4` 등 모델 인스턴스를 명시 제외하지만, 실험 섹션처럼 모델이 "주어"로 나오면 LLM이 규칙을 무시하고 뽑음(실측: §4 Experiments에서 `GPT-4` 2/2 누출). 운영맵에 모델 노드+난센스 엣지(`System 1 part_of GPT-4`) 박힘 = precision-first 위반.
**해결:** `agent_04_entity_extraction.py`에 `is_model_instance()` **결정적 후처리 필터** — 모델군 이름/버전 변형(`gpt-4`,`palm-540b`,`llama-2-70b` 등) 차단. 정확/버전 매칭(부분문자열 아님)이라 `BERTScore`·`ReAct` 등 진짜 개념은 보존. 프롬프트 규칙은 유지(대부분 거기서 걸림) + 필터가 누출분 보증.
**검증(결정적):** 단위 19/19(모델 차단 + 진짜개념 보존), 라이브 §4에서 `GPT-4` 제거 확인.
**남음:** `Act`←ReAct 류는 모델이 아니라 **입도(granularity)** 문제(별개). "전체 재실행=복권"의 프로세스 질문도 미해결.
