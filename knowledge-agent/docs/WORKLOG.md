...
## 2026-06-08 — 04 일반 원시어 필터 ("Act" 입도 노이즈)

**실측 먼저:** "Act"는 노이즈런의 우연인지 확인 — 방법섹션 9회 추출 0회, 전 11섹션 1회. **드물고 간헐적, 체계적 입도붕괴 아님.** 메서드를 분해한 일반 원시어(act/reasoning/thought)가 가끔 단독 누출 = GPT-4류와 같은 부류(프롬프트가 'reasoning processes' 제외 지시하나 샘).
**해결:** `agent_04`에 `_GENERIC_PRIMITIVES` + `is_noise_concept()`(모델 필터와 통합). 정확매칭이라 `thought generator`·`reasoning trace`·`Acting-only prompts`·`ReAct` 등 진짜 개념 보존. 리스트는 튜닝 가능.
**검증:** 단위 20/20(원시어 차단 + 진짜개념 보존).
**입도의 더 깊은 질문**(ToT 컴포넌트·task 변형이 적정 입도인가)은 **데이터상 안정적 정상**이라 fix 불필요 — 노이즈는 입도 *레벨*이 아니라 *recall 불안정*(어느 개념이 그 런에 뽑히나)에 있음 = "재실행 복권" 프로세스 이슈(별개).
