...
- 맵(`data/knowledge_map.json`): **5편 코퍼스**(CoT 2201.11903 / SC 2203.11171 / ReAct 2210.03629 / Reflexion 2303.11366 / ToT 2305.10601), **66노드 / 24엣지**. **단 파편화된 진단용** — 운영맵으로 신뢰 안 함(아래 "수렴 붕괴" 참조).
- ⭐ **#1 미해결 = cross-paper 수렴 붕괴.** 5편 실측에서 CoT 한 개념이 **6노드로 파편화**. 이게 지금 제일 근본 문제(프로젝트 목적=일관된 커버리지 맵을 정면 위협).
- 안정성 fix들(이름보기①·04b약어·모델필터·trim·RW제외)은 개별 케이스엔 유효했으나 **5편 규모에서 수렴은 못 막음.**

### ⭐ 수렴 붕괴 — 측정된 진단 (다음 세션 시작점)
증상: 같은 개념(CoT)이 논문마다 새 노드 → 6조각. 추측 말고 05 정렬 데이터 + judge 직접호출로 **원인 2개 확인**:
1. **이름 수식어로 judge가 갈라놓음.** 앵커가 `Chain-of-Thought Prompting`(CoT논문이 그 이름으로 뽑음)이라, 나중 `Chain of Thought`(bare)가 오면 judge가 "일반 vs prompting 기법 = DIFFERENT". → **①의 "수식어=다른것" 가드가 역효과.**
2. **top-2 crowding.** ReAct의 `Chain-of-thought prompting`은 앵커에 직접 붙이면 SAME인데 실제론 new — 그때 맵에 CoT 조각이 여러 개라 **임베딩 상위 2개에 진짜 앵커가 안 들어가** 판정조차 못 받음.
→ **악순환**: 한 번 놓침 → 조각 생성 → 조각이 다음 매칭 상위2 차지 → 또 놓침.
**판단:** 패치(judge 규칙·top-K)는 두더지잡기. 측정상 **(A) 패치는 안 버팀.** 근본은 **(B) 증분정렬 폐기 → 전 논문 개념을 한 번에 global clustering**(순서·눈덩이 의존 없음, "정체성은 코퍼스에서 떠오른다" 철학 구현). 큰 작업이라 다음 세션.
**기각된 오답:** "정의 1개 말고 전부로 매칭" — 측정상 임베딩은 후보를 이미 0.68~0.73으로 찾음. def-count 문제 아님.

### 부수 실측
- **trim_back_matter 버그 발견·수정**(`5da8f7d`): 부록 거대 논문(CoT 50섹션 중 References 14번째)에서 '절반 이후만' 가드가 부록을 못 잘라 62클러스터 폭증 → 강한경계 위치무관 절단으로 13섹션 정상화.
- **UL2 모델 누출** — 하드코딩 모델 블록리스트에 없음. **블록리스트 브리틀 재확인**(개념성=코퍼스재등장으로 풀자던 근거 강화).
- enrichment 부분작동: home 정의(`Chain-of-Thought Prompting` src=CoT논문)는 깨끗하나 파편 사이 묻힘.
- SELECT 기권 정상작동(오염 CoT/CoT-SC 파편에 저신뢰 플래그) → precision-first 유지.
...
## 2026-06-08 — 논문 3편 추가(5편 코퍼스) → cross-paper 수렴 붕괴 발견

**한 일:** CoT/Self-Consistency/Reflexion(arXiv 직접 다운, `data/raw_papers/`) 추가 → 5편을 현재 코드로 연대순 재구축. 추출은 병렬 백그라운드, 통합(05-08)은 맵 직렬.
**발견(핵심):** **cross-paper 수렴이 규모에서 붕괴** — CoT가 6노드로 파편화. 측정으로 원인 2개 확정(judge 수식어-갈라놓기 + top-2 crowding 악순환). 상세는 상단 "현재 상태 > 수렴 붕괴" 참조.
**부산물:** trim_back_matter 버그 수정(부록 거대논문), UL2 모델누출(블록리스트 브리틀 재확인).
**결론:** 패치는 안 버팀(측정). 근본 해법 = global clustering(증분정렬 폐기). 다음 세션 #1.
**상태:** 5편 맵은 파편화 진단용 — 운영맵 신뢰 안 함.
